...
;; Time-stamp:		<10 August 2000>
;; Keywords:		dired, sort, menu, dialogue
;; URL:			http://centaur.maths.qmw.ac.uk/Emacs/

;; $Id: dired-sort-menu.el,v 1.23 2000-08-10 15:21:41+01 fjw Exp $
...
;; This package also provides an alternative to the standard
;; `dired-mode' `mouse-2' binding at the end of the file.
...
(defun dired-sort-menu-items (&optional r)
  ...
  (list
   ...
   (vector
    ...
    :selected `(dired-sort-menu-switch-p "t" ,r) :active t)
 ...
 `("Sort By"
   ...
   ("Configuration"
    ...
    ["Restore Default" dired-sort-menu-restore-default
     ...
     :active t]
    ["Customize Saved" (customize-option 'dired-sort-menu-saved-config) t]
    "---"
    ["Dialogue Own Frame" (setq dired-sort-dialogue-own-frame
			       (not dired-sort-dialogue-own-frame))
     :style toggle
     :selected dired-sort-dialogue-own-frame :active t])
   ["Dialogue" dired-sort-dialogue t]
   ))
...
(defun dired-sort-menu-popup (event)
  ...
  (interactive "@e")
  (command-execute
   (lookup-key dired-sort-menu
	       (apply 'vector (x-popup-menu event dired-sort-menu)))))
...
This command *must* be run in the Dired buffer!"
  ...
  (let
    ...
    (widget-insert "_______________________\n\n ")
...
(defun dired-mouse-find-file (event)
  "In dired, visit *in full window* the file or directory you click on.
On a directory, open it in the *current* buffer.
Argument EVENT is what invoked this function."
  ;; Functionality suggested by Ian Swainson <Ian_Swainson@bausch.com>
  ;; Largely stolen from `dired.el'.
  ;; Alternative to the standard dired-mode mouse-2 binding.
  (interactive "e")
  (let (file)
    (save-excursion
...
;; Bind to Shift-Control-mouse-2, which is currently unused:
(define-key dired-mode-map [C-S-mouse-2] 'dired-mouse-find-file)
;; Ditto Meta-mouse-2:
(define-key dired-mode-map [M-mouse-2] 'dired-mouse-find-file)
;; (Could perhaps make better use of this key!)

;; To change some existing dired-mode key binding, e.g. mouse-2, put
;; something like this in your .emacs file instead of the `add-hook'
;; above:

;; (add-hook 'dired-load-hook
;;           (lambda () (require 'dired-sort-menu)
;;                      (dired-mode-map [mouse-2] 'dired-mouse-find-file)))
